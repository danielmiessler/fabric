...
# File handler using temp directory
temp_log_dir = os.path.join(tempfile.gettempdir(), 'fabric_logs')
os.makedirs(temp_log_dir, exist_ok=True)
log_file = os.path.join(temp_log_dir, f"fabric_ui_{datetime.now().strftime('%Y%m%d')}.log")
...
def safe_run_command(command: str) -> Tuple[str, str]:
    """Run command with proper fabric path."""
    try:
        module_path = get_fabric_path()
        if not module_path:
        ...
        args = command.replace('fabric', '').strip().split()
        full_command = [python_exe, module_path] + args
        
        logger.info(f"Environment PYTHONPATH: {env['PYTHONPATH']}")
        logger.info(f"Full command: {' '.join(full_command)}")
        
        result = run(full_command, capture_output=True, text=True, env=env)

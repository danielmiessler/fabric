	...
	for i, prompt := range request.Prompts {
		...
		default:
			...
			go func(p PromptRequest) {
				...
				// Process YouTube URLs if detected
				userInput := p.UserInput
				if isYouTubeURL(userInput) && h.youtube.IsConfigured() {
					// Extract video ID and get transcript
					videoId, playlistId, err := h.youtube.GetVideoOrPlaylistId(userInput)
					if err != nil {
						streamChan <- fmt.Sprintf("Error: Failed to process YouTube URL: %v", err)
						return
					}
					
					if videoId == "" && playlistId != "" {
						streamChan <- "Error: Playlist URLs are not supported, please provide a video URL"
						return
					}

					// Get video transcript
					transcript, err := h.youtube.GrabTranscript(videoId, "en")
					if err != nil {
						streamChan <- fmt.Sprintf("Error: Failed to get transcript: %v", err)
						return
					}

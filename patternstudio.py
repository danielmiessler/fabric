...
def star_output(log_index: int, custom_name: str = ""):
    ...
    if 0 <= log_index < len(st.session_state.output_logs):
        ...
        log_entry["custom_name"] = custom_name
...
def sanitize_input_content(input_text: str) -> str:
    ...
    text = ''.join(c if c in ['\n', '\t'] or not c.iscontrol() else ' ' for c in text)
...
def execute_patterns(patterns_to_run, chain_mode=False, initial_input=None):
    ...
    try:
        for pattern in patterns_to_run:
            ...
            try:
                ...
                try:
                    result = run(cmd, input=message.encode('utf-8'), capture_output=True, text=True, check=True)
...
def handle_star_name_input(log_index: int):
    """Handle the starring process when a name is input."""
    if st.session_state.temp_star_name:
        if star_output(log_index, st.session_state.temp_star_name):
            st.success("Output starred!")
                        ...
                        with col_load:
                            if st.button("📋 Load from Clipboard", use_container_width=True):
                                try:
                                    ...
                                    is_valid, error_message = validate_input_content(clipboard_content)
                                    if not is_valid:
                                        st.error(f"Invalid clipboard content: {error_message}")
                                    else:
                                        # Sanitize clipboard content
                                        sanitized_content = sanitize_input_content(clipboard_content)
                        ...
                        with col1:
                            if st.button("📋 Copy All Outputs"):
                                try:
                                    ...
                                    run(["xclip", "-selection", "clipboard"], input=all_outputs.encode(), check=True)
                ...
                if not st.session_state.output_logs:
                    ...
                    for i, log in enumerate(reversed(st.session_state.output_logs)):
                        with st.expander(
                            ...
                            st.markdown("### Input")
                            st.code(log["input"], language="text")  # Display input as code block
                            st.markdown("### Output")
                            st.markdown(log["output"])  # Display output as markdown
                            ...
                            with col1:
                                if not is_starred:
                                    if st.button("⭐ Star", key=f"star_{i}"):
                                        st.session_state.starring_output = i
                                        st.session_state.temp_star_name = ""
                                else:
                                    st.write("⭐ Starred")
                            ...
                            with col2:
                                if st.button("📋 Copy Output", key=f"copy_{i}"):
                                    try:
                                        run(["xclip", "-selection", "clipboard"], input=log["output"].encode(), check=True)
                                        st.success("Output copied to clipboard!")
                                    except Exception as e:
                                        st.error(f"Error copying to clipboard: {e}")
                
                # Handle starring process
                if st.session_state.starring_output is not None:
                    st.text_input(
                        "Enter a name for this output:",
                        key="temp_star_name",
                        value=st.session_state.temp_star_name,
                        on_change=lambda: handle_star_name_input(st.session_state.starring_output)
                    )
                    if st.button("Cancel Starring"):
                        st.session_state.starring_output = None
                        st.session_state.temp_star_name = ""
                        st.experimental_rerun()
